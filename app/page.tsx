...
function CountdownTimer({ deadline }: { deadline: Date }) {
  ...
  return (
    ...
    >
      ...
      <span className={`text-sm font-mono ${isEndingSoon ? "text-primary font-medium" : "text-white"}`}>
        {timeLeft}
      </span>
      <AnimatePresence>
        {isEndingSoon && (
          ...
          >
            <Badge className="text-xs bg-primary text-black font-bold animate-pulse-glow">Ends Soon</Badge>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        >
          <h2 className="text-4xl font-bold mb-2 text-white">Active Trading Boxes</h2>
          <motion.p
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white">{mockBoxes.length}</p>
              </div>
            </div>
          </motion.div>
          <motion.div
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white/60">Connect your wallet to start trading and creating boxes</p>
                </div>
              </div>
            </motion.div>
          )}
              ...
              >
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <motion.div
                      animate={hoveredCard === box.id ? { scale: 1.05 } : { scale: 1 }}
                      transition={{ type: "spring", stiffness: 300 }}
                    >
                      <CardTitle className="text-lg text-white">{box.tokenAmount}</CardTitle>
                    </motion.div>
                    <motion.div whileHover={{ scale: 1.1 }} transition={{ type: "spring", stiffness: 400 }}>
                      <Badge className="bg-primary text-black font-bold">{box.tokenType}</Badge>
                    </motion.div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-white">{box.marketPrice}</span>
                        <motion.div
                          animate={{
                            scale: box.priceChange > 0 ? [1, 1.2, 1] : [1, 1.1, 1],
                            rotate: box.priceChange > 0 ? [0, 5, 0] : [0, -5, 0],
                          }}
                          transition={{ duration: 2, repeat: Number.POSITIVE_INFINITY }}
                        >
                          {box.priceChange > 0 ? (
                            <div className="flex items-center gap-1 text-primary">
                              ...
                              <span className="text-xs font-bold">+{box.priceChange}%</span>
                            </div>
                          ) : (
                    ...
                    >
                      ...
                      <span className="text-sm font-medium text-primary">{box.marketPriceUSD}</span>
                    </motion.div>
                    ...
                    >
                      ...
                      <span className="text-sm text-white">{box.lastBidPrice}</span>
                    </motion.div>
                    ...
                    <motion.div
                      ...
                      transition={{ type: "spring", stiffness: 300 }}
                    >
                      <span className="text-sm text-white/60">Total Bidders</span>
                      <motion.span
